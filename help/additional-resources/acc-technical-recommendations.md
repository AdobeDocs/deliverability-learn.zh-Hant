...
將兩個IP位址12.34.56.78和12.34.56.79定義為授權傳送網域的電子郵件。 **~all**&#x200B;表示任何其他位址都應該解譯成SoftFail。
...
>[!NOTE]
>
>對於託管或混合式安裝，如果您已升級至[Enhanced MTA](https://experienceleague.adobe.com/docs/campaign-classic/using/sending-messages/sending-emails/sending-an-email/sending-with-enhanced-mta.html#sending-messages)，DKIM電子郵件驗證簽署會由Enhanced MTA針對所有網域的所有郵件完成。
...
* 如果您已設定Adobe Campaign執行個體的DomainKeys，您只需要在[網域管理規則](https://experienceleague.adobe.com/docs/campaign-classic/using/sending-messages/monitoring-deliveries/understanding-delivery-failures.html#email-management-rules)中選取&#x200B;**dkim**。 如果不適用，請遵循與DomainKeys （已取代DKIM）相同的設定步驟（私人/公開金鑰）。
...
* 加入URI取消訂閱連結。
* 支援從接收器接收HTTP POST回應，Adobe Campaign支援此功能。 您也可以使用外部服務。

若要直接在Adobe Campaign v7/v8中支援「一鍵式清單 — 取消訂閱POST」回應，您必須在「取消訂閱收件者no-click」網頁應用程式中新增。 若要這麼做：
...
Adobe為每位客戶提供專用的IP策略，並逐步提升IP，以建立聲譽並最佳化傳遞效能。
