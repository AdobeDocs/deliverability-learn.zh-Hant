...
例如，如果您已將&#x200B;*email.example.com*&#x200B;委派給Adobe以傳送電子郵件，Adobe將會建立子網域，如下所示：
...
若要在這些子網域上安裝SSL憑證，此程式包括請求CSR檔案，接著購買用於Adobe安裝或續約的SSL憑證。
...
在提交您的要求與相關資訊後，Adobe會產生憑證簽署要求(CSR)檔案並提供給您。
...
* 如果有一個或多個中間憑證，您必須向Adobe提供根憑證和所有中間憑證。
...
安裝SSL憑證並由Adobe客戶服務確認後，請確定所有URL皆已成功安裝SSL憑證。
...
您有責任檢查有效期間。 Adobe建議您實作程式來監控憑證到期日。 深入瞭解當您的SSL憑證於[本文](https://www.thesslstore.com/blog/what-happens-when-your-ssl-certificate-expires/)到期時會發生什麼事情。
